...
class TrendsService {
  ...
  Future<Map<String, dynamic>> getTrendsData(
    ...
    // Check cache first (must match filter and custom dates)
    if (_cachedData != null &&
        _lastCacheTime != null &&
    ...
    final results = await Future.wait([
      getExerciseFrequency(userId, filter, customStartDate: customStartDate, customEndDate: customEndDate),
      getProgressMetrics(userId, filter, customStartDate: customStartDate, customEndDate: customEndDate),
      getActivityCalendar(userId, filter, customStartDate: customStartDate, customEndDate: customEndDate),
      getSessionHistory(userId, filter, customStartDate: customStartDate, customEndDate: customEndDate),
    ]);
  ...
  Future<List<ExerciseFrequency>> getExerciseFrequency(
    ...
    for (var doc in snapshot.docs) {
      ...
      if (sessionDate.isBefore(startDate) || sessionDate.isAfter(endOfDay)) continue;
  ...
  Future<Map<String, List<ProgressMetric>>> getProgressMetrics(
    ...
    for (var doc in workoutSnapshot.docs) {
      ...
      if (workoutDate.isBefore(startDate) || workoutDate.isAfter(endOfDay)) continue;
